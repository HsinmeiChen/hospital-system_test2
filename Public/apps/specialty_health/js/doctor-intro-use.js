...
$('.open-stop-modal').on('click', function () {
    ...
    // Ajax 載入資料
    $.ajax({
        url: `/specialty_health/api/stop-info/${empId}/`,
        method: 'GET',
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    initAjaxCardPagination({
        apiUrl: `/specialty_health/api/doctor/${employeeId}/articles/`,
    ...
    // ========== 影音專區 ==========
    initAjaxCardPagination({
        apiUrl: `/specialty_health/video-section/${employeeId}/`,
...
$('#videoModal').on('show.bs.modal', function (event) {
    ...
    modal.find('#videoFrame').attr('src', url).attr('title', title);
});
...
document.addEventListener("DOMContentLoaded", function () {
    const currentDoctorId = "{{ employee_id }}";

    window.loadSidenav({
        apiUrl: '/specialty_health/api/doctor_sidenav/',
        containerId: 'doctor-sidenav',
        isActiveFn: item => item.employee_id === currentDoctorId,
        buildHrefFn: item => `/specialty_health/doctor/${item.employee_id}/`,
        renderTextFn: item => `<small>${item.department} ${item.job_title}</small><br>${item.name}`
    });
});
