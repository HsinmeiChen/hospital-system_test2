...
function loadRandomHealthReports() {
    fetch('/breast-care-center/api/random-breast-reports/')
        .then(res => res.json())
        .then(data => {
            const container = document.getElementById('random-breast-reports');
            if (!container) return;
            container.innerHTML = '';
            data.articles.forEach(article => {
                ...
                card.onclick = function() {
                    window.location.href = `/breast-care-center/articles/${article.filename}/`;
                };
...
function loadRandomHealthEdus() {
    fetch('/breast-care-center/api/random-breast-edus/')
        .then(res => res.json())
        .then(data => {
            const container = document.getElementById('random-breast-edus');
            if (!container) return;
            container.innerHTML = '';
            data.edus.forEach(edu => {
                ...
                // 建立 figure                
                const card = document.createElement('figure');
                card.className = 'card shadow-sm fade-in-card article-card';
                card.style.cursor = 'pointer';
                card.innerHTML = `
                    <!-- <div class="img-container">
                         <img src="${edu.thumb_image}" class="card-img-top" alt="${edu.title}">
                    </div> -->
                    <figcaption class="card-body d-flex align-items-center">
                        <h5 class="card-title" style="font-size:.9rem;">${edu.title}</h5>
                    </figcaption>
                `;

                // 點擊直接跳轉頁面
                card.onclick = function() {
                    window.location.href = `/breast-care-center/breast-edu/${edu.title_id}/`;
                };
